...
(function() {
  var activate, active, ajaxify, base_path_regexp, base_path_regexp_cache, base_paths, content_container, correct_url, display_loader, dont_correct_url, flash_types, get_content_container, ignore_hash_change, init, initial_history_state, is_string, load, load_page_from_hash, loader_element, on_ajaxify_success, on_hash_change, protocol_with_host, push_state, push_state_enabled, rails_ujs_fix, regexp_escape, reload_page_if_assets_stale, scroll_page_to_top, scroll_to_top, set_content_container, set_display_loader, set_loader_element, set_scroll_to_top, show_flashes, update_url, url_params_as_object;
  ...
  set_loader_element = function(new_loader_element) {
    return loader_element = new_loader_element;
  };
  ...
  init = function(options) {
    ...
    if ('display_loader' in options) {
      display_loader = options.display_loader;
    }
    ...
    if (!load_page_from_hash) {
      ...
      return $.ajax({
        ...
        beforeSend: function(xhr) {
          if (!('display_loader' in options)) {
            options.display_loader = display_loader;
          }
          if (options.display_loader) {
        ...
        error: function(data, status, jqXHR) {
          return $(document).trigger('ajaxify:load_error', [data, status, jqXHR, options.url]);
        }
      });
    }
  };
  ...
  on_hash_change = function() {
    ...
    if (base_paths && base_paths.length > 0 && (match = window.location.pathname.match(base_path_regexp()))) {
      url = match[0] + url;
    }
    if (url === '') {
      url = '/';
  ...
  set_display_loader = function($link_or_form) {
  ...
  reload_page_if_assets_stale = function(url, jqXHR) {
    var digest_header, meta_asset_digest;
    ...
    if (digest_header && meta_asset_digest !== '' && digest_header !== meta_asset_digest) {
  ...
  this.Ajaxify = {
    ...
    set_loader_element: set_loader_element,
